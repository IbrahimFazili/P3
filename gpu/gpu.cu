...
// Combined kernel to compute dh, du, dv on gpu w boundary checks
template <const uint BLOCKSIZE>
__global__ void compute_derivatives(double *d_dh, double *d_du, double *d_dv, double *d_h, double *d_u, double *d_v, double H, double g, double dx, double dy, int nx, int ny) {
    __shared__ double sh_h[BLOCKSIZE + 1][BLOCKSIZE + 1];
    __shared__ double sh_u[BLOCKSIZE + 1][BLOCKSIZE];
    __shared__ double sh_v[BLOCKSIZE][BLOCKSIZE + 1];

    const int i = blockIdx.x * BLOCKSIZE + threadIdx.x;
    const int j = blockIdx.y * BLOCKSIZE + threadIdx.y;

    // Load data into shared memory
    if (i < nx + 1 && j < ny + 1) {
        sh_h[threadIdx.x][threadIdx.y] = d_h[i * (ny + 1) + j];
    }
    if (i < nx + 1 && j < ny) {
        sh_u[threadIdx.x][threadIdx.y] = d_u[i * ny + j];
    }
    if (i < nx && j < ny + 1) {
        sh_v[threadIdx.x][threadIdx.y] = d_v[i * (ny + 1) + j];
    }
    __syncthreads();

    // Compute derivatives
    if (i < nx - 1 && j < ny - 1) { // Compute dh
        double du_dx = (sh_u[threadIdx.x + 1][threadIdx.y] - sh_u[threadIdx.x][threadIdx.y]) / dx;
        double dv_dy = (sh_v[threadIdx.x][threadIdx.y + 1] - sh_v[threadIdx.x][threadIdx.y]) / dy;
        d_dh[i * ny + j] = -H * (du_dx + dv_dy);
    }

    if (i < nx - 1 && j < ny) { // Compute du
        double dh_dx = (sh_h[threadIdx.x + 1][threadIdx.y] - sh_h[threadIdx.x][threadIdx.y]) / dx;
        d_du[i * ny + j] = -g * dh_dx;
    }

    if (i < nx && j < ny - 1) { // Compute dv
        double dh_dy = (sh_h[threadIdx.x][threadIdx.y + 1] - sh_h[threadIdx.x][threadIdx.y]) / dy;
        d_dv[i * ny + j] = -g * dh_dy;
    }
}
...
void step()
{
    ...
    // launch combined kernel to compute derivatives
    compute_derivatives<32><<<gridDim, blockDim>>>(d_dh, d_du, d_dv, d_h, d_u, d_v, H, g, dx, dy, nx, ny);
